...
import {
    changeProductVariantsPage,
    fetchProductCategories,
    fetchProductVariants,
    fetchSearchingProductVariants,
} from "api/products";
...
class Products extends PureComponent {
        ...
        if (category === "search") {
            ...
            if (category) {
                if (category && page) {
                    ...
                    if (price_filter) {
                        url = `product-variants/?category=${category}&page=${
                            page ? page : _page
                        }&price=${price_filter}`;
                    } else {
                        url = `product-variants/?category=${category}&page=${
                        ...
                        if (price_filter) {
                            ...
                            url = `product-variants/?category=${category}${
                                _page
                                    ...
                                    : ""
                            }&price=${price ? price : price_filter}`;
                        } else {
                            url = `product-variants/?category=${category}${
                                page ? `&page=${page}` : ""
                            }&price=${price}`;
    ...
    addToWishlist = (event, product_variant_id) => {
        ...
        setTimeout(() => {
            axiosWithAuthentication
                .post("wishlist/add/", wishlistData)
                .then((res) => {
                    this.fetchWishlist();
    ...
    removeWishlist = (event, product_id) => {
        ...
        try {
            wishlist = this.state.wishlist.find(
                (ws) => ws.product_variant.id === product_id
            );
        } catch (error) {}
        ...
        if (this.context) {
            if (product.in_wishlist) {
                return (
                    <span
                        onClick={(event) =>
                            this.removeWishlist(event, product.id)
                        }
                    >
                        <FaHeart
                            style={{ color: "rgb(229 4 80)" }}
                ...
                return (
                    <span
                        onClick={(event) =>
                            this.addToWishlist(event, product.id)
                        }
                    >
            ...
            return (
                <Link to={"/user/login/"} className="wishlist-remove-btn">
                    <FaHeart className="wishlist-remove-btn" />
                </Link>
            );
        }
            ...
            <>
                <div className="category-product-container">
                    <div className="categories-container">
                        <h5 className="text-center me-4 mb-3">
                            All Categories
                        </h5>
                        {productCategories &&
                    ...
                    <div className="products-container pt-4">
                        ...
                        {query && category === "search" && (
                            <h4 className="text-center mb-5 w-100">
                                searched for "{query}"
                            </h4>
                        )}
...
export default function FilterHandler() {
    ...
    return (
        <Products query={query} category={category} page={page} price={price} />
    );
}
